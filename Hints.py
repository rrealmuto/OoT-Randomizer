...
def add_hint(spoiler, world, groups, gossip_text, count, locations=[], force_reachable=False, *, hint_type):
    ...
    if hint_type in ('always', 'dual_always') and 'remove_always_stones' in world.hint_dist_user:
        ...
        for group in groups:
            ...
            if any(name in removed_stones for name in gossip_names):
...
def get_checked_areas(world, checked):
    def get_area_from_name(check):
        ...
        if location.type != 'Boss' or world.settings.shuffle_dungeon_rewards not in ('vanilla', 'reward'):
...
def get_goal_hint(spoiler, world, checked):
    ...
    prioritize_dungeon_hints = world.hint_dist_user.get('prioritize_dungeons', False)
    dungeon_category_locations = list(filter(lambda location: HintArea.at(location[0]).is_dungeon, category_locations))
    if prioritize_dungeon_hints and len(dungeon_category_locations) > 0:
        location_tuple = random.choice(dungeon_category_locations)
    else:
...
def buildWorldGossipHints(spoiler, world, checkedLocations=None):
    ...
    if 'dual_always' in hint_dist and hint_dist['dual_always'][1] > 0:
        ...
        for hint in alwaysDuals:
            ...
            add_hint(spoiler, world, stoneGroups, GossipText('%s #%s# and #%s#.' % (location_text, first_item_text, second_item_text), ['Red', 'Green', 'Green'], [firstLocation.name, secondLocation.name], [firstLocation.item.name, secondLocation.item.name]), hint_dist['dual_always'][1], [firstLocation, secondLocation], force_reachable=True, hint_type='dual_always')
    ...
    if hint_dist['always'][1] > 0:
        ...
        for hint in alwaysLocations:
            ...
            if '#' not in location_text:
                location_text = '#%s#' % location_text
            item_text = getHint(getItemGenericName(location.item), world.settings.clearer_hints).text
            add_hint(spoiler, world, stoneGroups, GossipText('%s #%s#.' % (location_text, item_text), ['Red', 'Green'], [location.name], [location.item.name]), hint_dist['always'][1], [location], force_reachable=True, hint_type='always')
    ...
    if world.entrance_shuffle and 'entrance_always' in hint_dist and hint_dist['entrance_always'][1] > 0:
        ...
        for entrance_hint in alwaysEntrances:
            ...
            if entrance.shuffled and (connected_region.dungeon or any(hint.name == connected_region.name for hint in getHintGroup('region', world))):
                ...
                add_hint(spoiler, world, stoneGroups, GossipText('%s %s.' % (entrance_text, region_text), ['Green', 'Light Blue']), hint_dist['entrance_always'][1], None, force_reachable=True, hint_type='entrance_always')

    # Add trial hints, only if hint copies > 0
    if hint_dist['trial'][1] > 0:
        if world.settings.trials_random and world.settings.trials == 6:
            add_hint(spoiler, world, stoneGroups, GossipText("#Ganon's Tower# is protected by a powerful barrier.", ['Pink']), hint_dist['trial'][1], force_reachable=True, hint_type='trial')
        elif world.settings.trials_random and world.settings.trials == 0:
            add_hint(spoiler, world, stoneGroups, GossipText("Sheik dispelled the barrier around #Ganon's Tower#.", ['Yellow']), hint_dist['trial'][1], force_reachable=True, hint_type='trial')
        elif world.settings.trials < 6 and world.settings.trials > 3:
            for trial,skipped in world.skipped_trials.items():
                if skipped:
                    add_hint(spoiler, world, stoneGroups,GossipText("the #%s Trial# was dispelled by Sheik." % trial, ['Yellow']), hint_dist['trial'][1], force_reachable=True, hint_type='trial')
        elif world.settings.trials <= 3 and world.settings.trials > 0:
            for trial,skipped in world.skipped_trials.items():
                if not skipped:
                    add_hint(spoiler, world, stoneGroups, GossipText("the #%s Trial# protects Ganon's Tower." % trial, ['Pink']), hint_dist['trial'][1], force_reachable=True, hint_type='trial')
    ...
    while stoneGroups:
        ...
        if hint == None:
            ...
            place_ok = add_hint(spoiler, world, stoneGroups, gossip_text, copies, locations, hint_type=hint_type)
            if place_ok:
    ...
    if world.settings.bridge == 'open':
        ...
        if world.settings.bridge == 'vanilla':
            ...
            count, singular, plural = {
                ...
                'hearts':     (world.settings.bridge_hearts,     "#heart#",                        "#hearts#"), #TODO clarify that this includes starting hearts
        ...
        if world.settings.shuffle_ganon_bosskey == 'on_lacs':
            if world.settings.lacs_condition == 'vanilla':
                ...
                count, singular, plural = {
                    ...
                    'hearts':     (world.settings.lacs_hearts,     "#heart#",                        "#hearts#"), #TODO clarify that this includes starting hearts
                }[world.settings.lacs_condition]
                item_req_string = f'{count} {singular if count == 1 else plural}'
            bk_location_string = f"provided by Zelda once {item_req_string} {'are' if plural else 'is'} retrieved"
            ...
            count, singular, plural = {
                ...
                'hearts':     (world.settings.ganon_bosskey_hearts,     "#heart#",                        "#hearts#"), #TODO clarify that this includes starting hearts
            }[world.settings.shuffle_ganon_bosskey]
            item_req_string = f'{count} {singular if count == 1 else plural}'
            bk_location_string = f"automatically granted once {item_req_string} {'are' if plural else 'is'} retrieved"
        else:
            ...
            if condition == 'triforce':
                if world.settings.triforce_hunt_mode == 'easter_egg_hunt':
                    ...
                    condition = 'ice'
            bk_location_string = getHint(f'ganonBK_{condition}', world.settings.clearer_hints).text
