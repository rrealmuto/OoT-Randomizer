...
void try_pending_item() {
    ...
    if ((override.value.base.item_id == 0xCA || (override.value.base.item_id >= 0x1000 && override.value.base.item_id < 0x1007)) && override.value.base.player != PLAYER_ID) { // Triforce piece
        uint16_t resolved_item_id = resolve_upgrades(override);
        item_row_t *item_row = get_item_row(resolved_item_id);
