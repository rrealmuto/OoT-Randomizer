...
collidercylinder_override collidercylinder_override_table[] = {
    { .scene = 0x58, .pad = 0, .setup = 2, .room = 1, .flag = 16, .radius_override = 70 }, // ZD Shop red ice
    { .scene = 0x09, .pad = 0, .setup = 0, .room = 3, .flag = 18, .radius_override = 70 },
    { .scene = 0x09, .pad = 0, .setup = 0, .room = 3, .flag = 19, .radius_override = 70 },
...
int32_t Collider_SetCylinder_Hook(z64_game_t* globalCtx, ColliderCylinder* collider, z64_actor_t* actor, ColliderCylinderInit* src) {
    ...
    while(curr->all)
    {
        ...
        {
            src->dim.radius = curr->radius_override;
        }
        curr++;
    }
    
    return Collider_SetCylinder(globalCtx, collider, actor, src);
}