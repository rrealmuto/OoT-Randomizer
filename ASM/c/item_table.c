...
item_row_t item_table[] = {
    ...
    [0x0071] = ITEM_ROW(0x53,      SILVER_CHEST, 0x41, -1, 0x00F3, 0x00AA, 0x02, no_upgrade, give_small_key, TCG_ID, -1, resolve_text_small_keys_cmg), // Small Key (Chest Game)
    ...
    [0x0127] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x0080, 0x01AB, 0x9C, no_upgrade, give_quest_item, 18, -1, NULL), // Kokiri Emerald
    [0x0128] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x0081, 0x01AC, 0x9D, no_upgrade, give_quest_item, 19, -1, NULL), // Goron Ruby
    [0x0129] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x0082, 0x01AD, 0x9E, no_upgrade, give_quest_item, 20, -1, NULL), // Zora Sapphire
    [0x012A] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x0040, 0x01AE, 0x96, no_upgrade, give_quest_item,  5, -1, NULL), // Light Medallion
    [0x012B] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x003E, 0x01AF, 0x97, no_upgrade, give_quest_item,  0, -1, NULL), // Forest Medallion
    [0x012C] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x003C, 0x01B0, 0x98, no_upgrade, give_quest_item,  1, -1, NULL), // Fire Medallion
    [0x012D] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x003D, 0x01B1, 0x99, no_upgrade, give_quest_item,  2, -1, NULL), // Water Medallion
    [0x012E] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x0041, 0x01B2, 0x9A, no_upgrade, give_quest_item,  4, -1, NULL), // Shadow Medallion
    [0x012F] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x003F, 0x01B3, 0x9B, no_upgrade, give_quest_item,  3, -1, NULL), // Spirit Medallion

    // New items in dev-fenhl which are not in main Dev
    // Some previously used IDs may be skipped, to simplify auto-tracker support

    [0x1000] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x9057, 0x01B5, 0x29, no_upgrade, give_triforce_piece, -1, -1, NULL), // Easter egg (pink)
    [0x1001] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x9057, 0x01B6, 0x29, no_upgrade, give_triforce_piece, -1, -1, NULL), // Easter egg (orange)
    [0x1002] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x9057, 0x01B7, 0x29, no_upgrade, give_triforce_piece, -1, -1, NULL), // Easter egg (green)
    [0x1003] = ITEM_ROW(0x53,      GILDED_CHEST, 0x41, -1, 0x9057, 0x01B8, 0x29, no_upgrade, give_triforce_piece, -1, -1, NULL), // Easter egg (blue)
    ...
    [0x1016] = ITEM_ROW(0x53,      SILVER_CHEST, 0x41, -1, 0x9213, 0x0195, 0x77, no_upgrade, give_small_key_ring, WATER_ID,  true, NULL), // Water Temple Key Ring (with boss key)
    [0x1017] = ITEM_ROW(0x53,      SILVER_CHEST, 0x41, -1, 0x9214, 0x0195, 0x77, no_upgrade, give_small_key_ring, SPIRIT_ID, true, NULL), // Spirit Temple Key Ring (with boss key)
    [0x1018] = ITEM_ROW(0x53,      SILVER_CHEST, 0x41, -1, 0x9215, 0x0195, 0x77, no_upgrade, give_small_key_ring, SHADOW_ID, true, NULL), // Shadow Temple Key Ring (with boss key)

    [0x1019] = ITEM_ROW(0x53,      GILDED_CHEST, 0x0C, -1, 0x9054, 0x0158, 0x61, no_upgrade, no_effect, -1, -1, NULL), // Blue Fire Arrow

    [0x101A] = ITEM_ROW(0x5B,   SKULL_CHEST_BIG, 0x71, -1, 0x00B4, 0x015C, 0x63, no_upgrade,         no_effect,           -1, -1, resolve_text_skull_token), // Gold Skulltula Token (big chest)
    [0x101B] = ITEM_ROW(0x3D,   HEART_CHEST_BIG, 0x72, -1, 0x00C6, 0x00BD, 0x13, health_upgrade_cap, clear_excess_hearts, -1, -1, NULL), // Heart Container (big chest)
    [0x101C] = ITEM_ROW(0x3E,   HEART_CHEST_BIG, 0x7A, -1, 0x00C2, 0x00BD, 0x14, health_upgrade_cap, full_heal,           -1, -1, NULL), // Piece of Heart (big chest)
