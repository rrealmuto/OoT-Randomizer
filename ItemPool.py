...
reward_list = [item.name for item in sorted([i for n, i in ItemInfo.items.items() if i.type == 'DungeonReward'], key=lambda x: x.special['item_id'])]
...
def get_pool_core(world):
    ...
    for location in world.get_locations():
        ...
        # Bombchus
            ...
            if world.settings.bombchus_in_logic:
                item = 'Bombchus'
            shuffle_item = world.settings.shuffle_base_item_pool and (location.name != 'Wasteland Bombchu Salesman' or world.settings.shuffle_medigoron_carpet_salesman)
    ...
    if world.settings.shuffle_smallkeys == 'vanilla':
        ...
        if 'Shadow Temple' in world.settings.dungeon_shortcuts:
            ...
            world.state.collect(ItemFactory('Small Key (Shadow Temple)'))
        elif world.mixed_pools_bosses and not world.dungeon_mq['Shadow Temple']:
