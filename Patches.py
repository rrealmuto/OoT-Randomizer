...
def patch_rom(spoiler:Spoiler, world:World, rom:Rom):
    ...
    already_checked = {region.name for region in jabu_reward_regions}
    location = None
    while jabu_reward_regions:
        region = jabu_reward_regions.pop(0)
        locations = list(filter(lambda loc: not loc.locked and loc.has_item() and not loc.item.event, region.locations))
        if locations:
        ...
        for exit in region.exits:
            if exit.connected_region.dungeon != 'Jabu Jabus Belly' and exit.connected_region.name not in already_checked:
                jabu_reward_regions.append(exit.connected_region)
                already_checked.add(exit.connected_region.name)
