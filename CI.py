    ...
    with open(data_path('presets_default.json'), encoding='utf-8') as f:
        ...
        for trick in logic_tricks.values():
            if trick['name'] not in presets['Hell Mode']['allowed_tricks']:
                error(f'Logic trick {trick["name"]!r} missing from Hell Mode preset.', False) #TODO automatic fix via update-presets.py
    ...
    if hasattr(error, "count") and error.count:
        ...
        if fix_errors:
            if getattr(error, "cannot_fix", False):
                print(f'Some errors could not be fixed automatically.', file=sys.stderr)
                sys.exit(1)
            else:
                print(f'All errors fixed.', file=sys.stderr)
                sys.exit(0)
        else:
            if getattr(error, "can_fix", False):
                if getattr(error, "cannot_fix", False):
                    print(f'Run `CI.py --fix` to automatically fix some of these errors.', file=sys.stderr)
                else:
                    print(f'Run `CI.py --fix` to automatically fix these errors.', file=sys.stderr)
            sys.exit(1)
    else:
